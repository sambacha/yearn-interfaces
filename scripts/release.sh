...
publish() {
  ...
  log "Publishing yfi-solidity on npm"
  npm publish --tag "$dist_tag" --otp "$(prompt_otp)"

  log "Publishing @yfi/interface-contracts on npm"
  env ALREADY_COMPILED= \
      npm publish interface-contracts --tag "$dist_tag" --otp "$(prompt_otp)"

  if [[ "$dist_tag" == "latest" ]]; then
    otp="$(prompt_otp)"
    npm dist-tag rm --otp "$otp" yfi-solidity next
    npm dist-tag rm --otp "$otp" @yfi/interface-contracts next
  fi
}
